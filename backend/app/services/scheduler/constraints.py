...
def getMaxContinuousWorkMinutes(constraints: list[Constraint]) -> Optional[int]:
    """Return the strictest max continuous work limit in minutes, or None."""
    values = []
...
def validateWithinAvailability(blocks: list[dict], windows: list[dict]) -> None:
    """Assert every block is fully inside at least one availability window."""
    for b in blocks:
...
def validateNoProtectedOverlap(blocks: list[dict], protectedConstraints: list[dict]) -> None:
    """Assert no block overlaps protected_block constraints."""
    for b in blocks:
        start = b.get("start_time")
        end = b.get("end_time")
        if isinstance(start, str):
        ...
        for pc in protectedConstraints:
            if pc.get("constraint_type") != "protected_block":
                continue
            c = Constraint(constraint_type="protected_block", day_of_week=pc.get("day_of_week"), start_minutes=pc.get("start_minutes"), end_minutes=pc.get("end_minutes"))

...
def test_invariants_max_continuous_work_non_splittable(client: TestClient):
    ...
    longFixedBlocks = []
    for block in blocks:
        if block["task_name"] == "Long Fixed":
            longFixedBlocks.append(block)
    assert len(longFixedBlocks) >= 2
    totalMins = 0
    for block in longFixedBlocks:
        totalMins += block["duration_minutes"]
    assert totalMins == 90
...
def test_move_block_rejected_when_overlapping(client: TestClient):
    ...
    blockB = None
    for block in blocks:
        if block["task_name"] == "A":
            blockA = block
        if block["task_name"] == "B":
            blockB = block
    assert blockA is not None
    assert blockB is not None
